...
class Vbflasher:
	def __init__(self, can_interface="can0", sbl_path=None, exe_path=None, data_path=None):
		...
		if self.ecuid:
			...
			# tc/qdisc istnieje tylko na Linuxie z SocketCAN.
			if platform.system() != "Darwin" and not tccheck(can_interface):
...
if __name__ == '__main__':
	...
	try:
		try:
			...
			if enum == 99:
				die('[!] Unable to assign ecu address')
			die(e)
		...
		if flasher is not None and getattr(flasher, "ecu", None) is not None:

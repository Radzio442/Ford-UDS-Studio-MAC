...
class Ecu:
	...
	def close(self):
		"""Zamyka połączenie CAN/ISO-TP."""
		if getattr(self, "ssock", None) is not None:
			self.ssock.close()
	...
	def UDSTransferDataEx(self, num, data=bytearray()):
		...
		while True:
			...
			if not msg:
				return False
			# NRC 0x78 = Response Pending. Czekamy na końcową odpowiedź.
			if len(msg) >= 3 and msg[0] == 0x7F and msg[1] == 0x36 and msg[2] == 0x78:
				continue
			if msg[0] == 0x76 and len(msg) >= 2 and msg[1] == (num & 0xFF):
				return msg[2:]
			return False

	def UDSUploadBlock(self, num):
		"""Żąda kolejnego bloku RequestUpload (36 xx) i zwraca jego dane."""
		return self.UDSTransferDataEx(num, bytearray())
	...
	def getHWPartNo(self):
		s = self.UDSReadDataByIdentifier([0xf1, 0x11])
		return s.decode('utf-8', errors='replace').strip('\x00') if s else False

	def getPartNo(self):
		s = self.UDSReadDataByIdentifier([0xf1, 0x13])
		return s.decode('utf-8', errors='replace').strip('\x00') if s else False
